...
class StatsScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final transactionStream = repository.watchActiveTransactionsInRange(
      startIsoInclusive: _iso(start6),
      endIsoExclusive: _iso(endExclusive),
      categoryId: null,
    );

    final categoryStream = repository.watchActiveCategories();
    ...
    return Scaffold(
      ...
      body: StreamBuilder<List<FinanceCategory>>(
        ...
        builder: (context, categorySnap) {
          final categoryById = {
            for (final category
                in categorySnap.data ?? const <FinanceCategory>[])
          ...
          return StreamBuilder<List<FinanceTransaction>>(
            ...
            builder: (context, transactionSnap) {
              final items =
                  transactionSnap.data ?? const <FinanceTransaction>[];
              if (items.isEmpty) {
                return const Center(
              ...
              for (final t in items) {
                ...
                if (incomeByMonth.containsKey(key) ||
                  ...
                  if (t.type == 'income') {
                    incomeByMonth[key] =
                        (incomeByMonth[key] ?? 0) + t.amountCents;
                  } else {
                    expenseByMonth[key] =
                ...
                if (key == thisMonthKey && t.type == 'expense') {
                  final cat = t.categoryId != null && t.categoryId!.isNotEmpty
                      ? categoryById[t.categoryId] ?? t.category
                      : t.category;
